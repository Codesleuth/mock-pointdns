...
function authModule(req, res, next) {
  ...
  if (auth === undefined) {
    debug('Authorization header missing in request.');
    if (req.url == '/') {
      return res
  ...
  debug('Authorization header incorrect in request.');
...
module.exports = {
  app: app,
  addZone: function (zone, records) {
    zone = zone || random.zone();
  ...
  getRecord: function (zoneId, recordId) {
